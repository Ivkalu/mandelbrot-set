...
//za raèunanje koliko iteracija treba dok ne eksplodira u beskonaènost
...
{
	double maxiter = MAXITER + zoomlvl / 5; //poveæavanje maksimalnog broja iteracija ovisno o dubini zumiranja
	if (iteracija > (maxiter))
		return 0; //ako je unutar mandelbrota, vraæa se 0 (crna boja)
...
int main(int argc, char* argv[])
{
	...
	int x; //koordinate miša na ekranu
	int y;
	int xprosli = 0; //prijašnje koordinate miša
	int yprosli = 0;
	...
	long double X = 0; //koordinate miša u kompleksnoj ravnini
	long double Y = 0;

	bool pressed = false; //je li miš pritisnut

	mandelbrot(xcentar, ycentar, radijus); //uèitavanje poèetnog mandelbrota
	...
	while (true) {
		...
		xprosli = x; //pamæenje prijašnjih koordinata miša
		yprosli = y;
